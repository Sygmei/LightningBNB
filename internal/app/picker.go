...
func (m pickerModel) View() string {
	return renderPickerView(m.output, m.orderedIDs, m.devices, m.selected, m.scanning, m.frame, errorString(m.scanErr))
}
...
func newPickerStyles(output io.Writer) pickerStyles {
	renderer := lipgloss.NewRenderer(output)
	colored := os.Getenv("NO_COLOR") == ""
	styles := pickerStyles{
		frame:    renderer.NewStyle().Border(lipgloss.RoundedBorder()).BorderForeground(lipgloss.Color("#334155")).Padding(1, 2).Width(78),
...
func renderPickerView(output io.Writer, orderedIDs []string, devices map[string]ble.Device, selected int, scanning bool, frame int, errorText string) string {
	...
	if errorText != "" {
		...
		for index, id := range orderedIDs {
			...
			result.WriteString("   " + styles.render(styles.muted, "ID "+serverID) + "\n")
		}
	}
	result.WriteString("\n" + styles.render(styles.muted, "↑/↓ or j/k move · Enter connect · q cancel"))
